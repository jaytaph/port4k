...
use port4k_server::renderer::{render, render_with_opts, RenderOptions, MissingVarPolicy};
...
fn var_basic_and_default() {
    ...
    assert_eq!(render("Hello {v:name}!", &v), "Hello Nova!");
    assert_eq!(render("Hello {v:missing:World}!", &v), "Hello World!");
}
...
fn var_missing_policy() {
    let v = HashMap::new();
    // default: LeaveToken
    assert_eq!(render("X{v:who}Y", &v), "X{v:who}Y");

    let s = render_with_opts("X{v:who}Y", &v, &RenderOptions { missing_var: MissingVarPolicy::Empty });
    assert_eq!(s, "XY");

    let s = render_with_opts("X{v:who}Y", &v, &RenderOptions { missing_var: MissingVarPolicy::Undefined });
...
fn var_string_format_padding() {
    let v = vars(&[("name", "Ada")]);
    assert_eq!(render("-{v:name|%-6s}-", &v), "-Ada   -"); // left-align, pad right
    assert_eq!(render("-{v:name|%6s}-", &v),  "-   Ada-"); // right-align, pad left
    assert_eq!(render("-{v:name|%3s}-", &v),  "-Ada-");    // equal width
    assert_eq!(render("-{v:name|%2s}-", &v),  "-Ada-");    // shorter than content
}

#[test]
fn var_int_format_padding_and_zero() {
    let v = vars(&[("score", "7"), ("big", "12345")]);
    assert_eq!(render("S={v:score|%5d}", &v),  "S=    7");
    assert_eq!(render("S={v:score|%05d}", &v), "S=00007");
    assert_eq!(render("B={v:big|%5d}", &v),    "B=12345");
    assert_eq!(render("B={v:big|%03d}", &v),   "B=12345"); // width <= len
}
...
fn var_int_parse_fallback_to_zero() {
    ...
    assert_eq!(render("N={v:weird|%05d}", &v), "N=00000");
}
...
fn var_default_can_contain_pipes() {
    ...
    let v = HashMap::new();
    let s = render("-{v:title:alpha | beta | gamma|%25s}-", &v);
...
fn color_fg_bg_attrs_any_order() {
    ...
    let s = render("{c:yellow:red:bold,underline}ALERT{c}", &v);
...
fn color_attr_second_field_is_attrs_not_bg() {
    ...
    let s = render("{c:yellow:bold}Y{c}", &v);
    // yellow + bold present
...
fn escapes_and_unknown_tokens() {
    ...
    // Escapes
    assert_eq!(render("{{}}", &v), "{}");
    assert_eq!(render("{{v}} -> {v:name}", &v), "{v} -> {v:name}");
    // Unknown token passthrough
    assert_eq!(render("X{x:foo}Y", &v), "X{x:foo}Y");
}

#[test]
fn unterminated_brace_is_literal() {
    let v = vars(&[("name", "Nova")]);
    assert_eq!(render("Hello {v:name", &v), "Hello {v:name");
}

#[test]
fn multiple_tokens_sequence() {
    let v = vars(&[("a","1"),("b","2"),("c","3")]);
    let s = render("{v:a}{v:b}{v:c}", &v);
    assert_eq!(s, "123");
}
