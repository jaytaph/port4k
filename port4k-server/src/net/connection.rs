...
use crate::util::telnet::{telnet_echo_on, telnet_echo_off, read_telnet_line};
use crate::state::registry::Registry;
use crate::state::session::{Session, ConnState};
...
pub async fn handle_connection(
    ...
    loop {
        ...
        if sess.lock().await.editor.is_some() {
            ...
            if !out.is_empty() { w.write_all(out.as_bytes()).await?; }
            ...
            if parts.len() == 1 {
                if sess.lock().await.state == ConnState::LoggedIn {
                    ...
                    if !registry.user_exists(&user).await {
                        ...
                        if pw.is_empty() { telnet_echo_on(&mut w).await?; return Ok(()); }
